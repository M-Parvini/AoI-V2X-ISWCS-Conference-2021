...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, size_platoon, n_RB,
        ...
        self.V2V_demand_size = V2V_SIZE  # V2V payload: 800 Bytes every 100 ms
    ...
    def act_for_training(self, actions):
        ...
        for i in range(int(self.n_Veh / self.size_platoon)):
            per_user_reward[i] = (-4.95)*(Demand[i]/self.V2V_demand_size) - platoon_AoI[i]/20 + \
                                 (0.05)*self.Revenue_function(C_rate[i], self.V2I_min)
